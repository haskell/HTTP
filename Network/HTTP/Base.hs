...
#define MONAD_COMPAT Fail.MonadFail
#else
#define MONAD_COMPAT Monad
#endif
...
failHTTPS :: MONAD_COMPAT m => URI -> m ()
#if MIN_VERSION_base(4,9,0)
failHTTPS uri
  | map toLower (uriScheme uri) == "https:" = Fail.fail "https not supported"
  | otherwise = return ()
#else
failHTTPS uri
...
-- header.
getAuth :: MONAD_COMPAT m => Request ty -> m URIAuthority
getAuth r =
